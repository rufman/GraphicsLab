...
void PLYModel::processFile( const std::string &file )
{
	...
	// Read file type
	std::string fileType; f >> fileType;
	if ( fileType != "ply" )
	{
		std::stringstream error; error << "File type " << fileType << " not recognized";
		throw Exception( error.str() );
	}

	// Read format: binary/ascii, litte/big endian
	std::string format, mode, version; f >> format >> mode >> version;
	if ( ( format != "format" ) || ( ( mode != "ascii" ) && ( mode != "binary_big_endian" ) && ( mode != "binary_little_endian" ) ) )
	{
		std::stringstream error; error << "Format specification invalid: \"" << format << " " << mode << "\"";
		throw Exception( error.str() );
	}
	...
	for ( uint vertexID = 0; vertexID < numVertices; vertexID++ )
	{
		...
		if ( x < _plyData.min.x ) _plyData.min.x = x;
		if ( y < _plyData.min.y ) _plyData.min.y = y;
		if ( z < _plyData.min.z ) _plyData.min.z = z;

		if ( x > _plyData.max.x ) _plyData.max.x = x;
		if ( y > _plyData.max.y ) _plyData.max.y = y;
		if ( z > _plyData.max.z ) _plyData.max.z = z;
	}

	// Scale vertices to be in [0, 1] and y >= 0
	float scaleX = 1.0/(_plyData.max.x - _plyData.min.x ),
	scaleY = 1.0/(_plyData.max.y - _plyData.min.y ),
	scaleZ = 1.0/(_plyData.max.z - _plyData.min.z );
	float scale = ( scaleX < scaleY ) ?
	( ( scaleZ < scaleX ) ? scaleZ : scaleX ) : ( ( scaleZ < scaleY ) ? scaleZ : scaleY );
	for ( uint vertexID = 0; vertexID < numVertices; vertexID++ )
	{
	...
	{
		uint num, vertex1, vertex2, vertex3; f >> num >> vertex1 >> vertex2 >> vertex3;
