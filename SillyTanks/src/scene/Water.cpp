...
float Water::getHeight(const Point &point) const {
	...
	for (int i = 0; i < numTriangles; i++) {
		Vector3D A = Vector3D(_vertices[_triangles[i].vertex1].x,
				_vertices[_triangles[i].vertex1].z, 0);
		Vector3D B = Vector3D(_vertices[_triangles[i].vertex2].x,
				_vertices[_triangles[i].vertex2].z, 0);
		Vector3D C = Vector3D(_vertices[_triangles[i].vertex3].x,
				_vertices[_triangles[i].vertex3].z, 0);
	...
	point_y = (_triangleNormals[triangle_id].x
			* (point.x - _vertices[_triangles[triangle_id].vertex1].x)
			+ _triangleNormals[triangle_id].z
					* (point.z - _vertices[_triangles[triangle_id].vertex1].z))
			/ -_triangleNormals[triangle_id].y
			+ _vertices[_triangles[triangle_id].vertex1].y;
...
Vector3D Water::getNormal(const Point &point) const {
	...
	return Vector3D(_triangleNormals[triangle_id].x,
			_triangleNormals[triangle_id].y, _triangleNormals[triangle_id].z);
}
