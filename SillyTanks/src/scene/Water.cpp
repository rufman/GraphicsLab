...
float Water::getHeight( const Point &point ) const
{
	...
	point_y = (_triangleNormals[triangle_id].x*(point.x-_vertices[_triangles[triangle_id].vertex1].x) +
			   _triangleNormals[triangle_id].z*(point.z-_vertices[_triangles[triangle_id].vertex1].z))/-_triangleNormals[triangle_id].y + _vertices[_triangles[triangle_id].vertex1].y;
...
void Water::update( float seconds )
{
	...
	if ( _displacement > ( 2*Utils::PI ) ) _displacement -= 2*Utils::PI;	
...
void Water::draw() const
{
    ...
    // Sea surface material
    float materialAmbient[3] = { 0.1, 0.1, 0.2 };
	float materialDiffuse[3] = { 0.2, 0.2, 0.4 };
    float materialSpecular[3] = { 0.4, 0.4, 0.6 };
